...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、十六个文件的十九个 pointer 来源共 63 个事件/117 条路径，并建立 48 个 `已盘点` 用例；当前关联 81 配置键、58 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`ONLINE-YIKE-001` 已补入 WebView 安全/生命周期边界，`TSUMEGO-001` 已补入屏幕取题权限/进程/棋盘事务边界，`UI-TEXT-EDIT-001` 已覆盖共享文本控件及 JPaintTextPane 的纯文本/HTML/缓存边界，`UI-BLUNDER-SCROLLBAR-001` 已覆盖 DemoScrollBarUI2 上下箭头 hover、侧栏/config/SGF/问题列表副作用及原生滚动条目标；下一步盘点 `WindowMenuStrip.java`，之后处理其余输入文件，再继续 515 配置键、325 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `WindowMenuStrip.java` 的 pointer 与菜单窗口安全边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
