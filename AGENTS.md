...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及五个键源的 128 个 case/349 条路径和三个 pointer 源的 16 个事件/39 条路径，并建立 43 个 `已盘点` 用例；当前关联 36 配置键、48 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；提交 `70f78ae2b6a1fe4916ae04c6f0a8a61abdd037d2` 已推送到 draft PR `#2`，补入子棋盘输入；提交 `2b658119131641850f1697a64a2bba87bddabc99` 已推送到同一 PR，补入 FloatBoard 键盘、鼠标按下/离开、移动和滚轮入口及 `BOARD-FLOAT-INPUT-001`；下一步盘点同时含键盘和鼠标入口的 `AnalysisFrame.java`，之后处理其余输入文件，再继续 560 配置键、335 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
