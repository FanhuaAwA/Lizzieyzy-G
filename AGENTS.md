...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、二十九个文件的八十二个 pointer 来源共 179 个事件/598 条可达路径，并建立 54 个 `已盘点` 用例；当前关联 211 配置键、101 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`MoveListFrame.java` 的 7 个索引来源完整覆盖三表 click、match panel press/exit/move/drag/wheel 与三表头 release 共 11 个 public pointer handler/41 条可达路径，贯通单双引擎窗口、统计/表格/图表、筛选排序、棋步导航与导出，并登记 Timer/窗口泄漏、自动分析/PK/页签门、排序不恢复、同步非原子导出、逐像素历史遍历、表格重复排序、刷新间隔、多屏/DPI/键盘/无障碍和资源边界；下一步唯一盘点 `RightClickMenu.java` 的 pointer 路径，之后处理六个键事件文件，再继续 385 配置键、282 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，唯一盘点 `RightClickMenu.java` 的 pointer 路径与菜单安全边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
