...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、二十三个文件的三十一个 pointer 来源共 88 个事件/169 条路径，并建立 49 个 `已盘点` 用例；当前关联 127 配置键、63 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`UI-LAYOUT-001` 已补入 IndependentSubBoard 的锁定/置顶/关闭 hover、窗口拖放/缩放、候选副作用、边界恢复、生命周期、渲染资源和无障碍边界，`ONLINE-YIKE-001` 已补入 WebView 与 YikeLiveDialog 列表请求/取消竞态、分页、输入、代理、同步改盘/分析、窗口/网络/资源上限和无障碍边界，`TSUMEGO-001` 已补入屏幕取题权限/进程/棋盘事务边界，`UI-TEXT-EDIT-001` 已覆盖共享文本控件及 JPaintTextPane 的纯文本/HTML/缓存边界，`UI-BLUNDER-SCROLLBAR-001` 已覆盖 DemoScrollBarUI2 箭头 hover、BlunderListPanel 指针/问题手快照/导航及 SidebarHeaderPanel 的模式/筛选命中、重复副作用、父容器主题、键盘/焦点/无障碍/lifecycle 边界，`UI-WINDOW-MENU-STRIP-001` 已覆盖顶级菜单壳的打开/关闭/悬停切换、陈旧菜单、重复通知、输入抑制、焦点和 listener 生命周期边界，`UI-DETAILED-TOOLBAR-001` 已补入 BottomToolbar 背景点击强制 18 字段失焦、四个空事件、F6/IME/校验/51 槽持久化及布局/DPI/无障碍边界；`SETTINGS-GENERAL-001` 与 `SETTINGS-THEME-001` 已补入 ConfigDialog2 的七项侧栏、26 个开关行、十个主题色块和问题手颜色编辑，以及取消不回滚、timer/owner/modal/ponder、分步保存、布局/DPI/键盘/无障碍边界；下一步盘点 `IndependentMainBoard.java`，之后处理其余输入文件，再继续 469 配置键、320 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
