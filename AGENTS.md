...
- T-003 按“活跃字面量叶键 + 明确持久化 JSON 接收者”的当前静态口径检出 596 个配置键和 1,627 处引用，现有矩阵关联 181 个键；另有 37 个仅出现在 `Config.java` 注释中的历史候选键；其余 415 个字面量键、嵌套规范路径、计算键和逐用例语义归属仍未完成。
- T-003 按 `Menu.java` 中活跃的 `Menu.*`/`menu.*` 资源读取检出 383 个菜单资源键和 473 处引用，现有矩阵关联 99 个键；12 个活跃硬编码菜单字面量和两个显式 `setAccelerator`（`Shift+O`、仅 Windows 的 `Alt+O`）均已全部反向关联。十四个文件的十五个已纳入键源合计 143/143 个活跃 `VK_*` case、365/365 条路径；二十八个文件的七十五个 pointer 源合计 168/168 个事件、557/557 条可达路径；全部反向关联 53 行矩阵并保留来源、修饰键、条件方法副作用、早退、空监听/空动作、执行语句、switch fall-through、数据依赖循环和原子 try/catch。代码库另有 6 个含键事件方法的文件和 2 个含鼠标事件方法的文件未纳入；其余 284 个菜单资源键也仍待盘点。
...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、三十个文件的八十三个 pointer 来源共 180 个事件/599 条可达路径，并建立 54 个 `已盘点` 用例；当前关联 214 配置键、101 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径，旧源码中的 active public pointer handler 已全部纳入。`RightClickMenu.java` 的唯一 `mouseExited` 是显式空操作；真实空点菜单由 PopupMenuListener/ActionListener 驱动，已贯通主/独立棋盘分流、编辑/分支/allow/avoid/KataGo Custom/追踪引擎与三个 tracking 配置，并登记 BUTTON3 跨平台缺口、共享静态坐标、非 daemon Timer、字符串身份比较、allow3 删除错误、非事务棋谱/引擎状态、无真实 warning/取消、visits 溢出/资源上限、DPI/键盘/触控/无障碍和零入口级自动化测试。下一步唯一盘点剩余六个键事件文件中的 `BottomToolbar.java`，之后依次处理 `GtpConsolePane.java`、`LizzieFrame.java`、`Menu.java`、`MoveListFrame.java`、`TsumeGoFrame.java`，再继续 382 配置键、282 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，唯一盘点 `BottomToolbar.java` 的键事件路径与工具栏安全边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
