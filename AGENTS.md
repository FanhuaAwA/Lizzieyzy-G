...
- T-003 按“活跃字面量叶键 + 明确持久化 JSON 接收者”的当前静态口径检出 596 个配置键和 1,627 处引用，现有矩阵关联 146 个键；另有 37 个仅出现在 `Config.java` 注释中的历史候选键；其余 450 个字面量键、嵌套规范路径、计算键和逐用例语义归属仍未完成。
- T-003 按 `Menu.java` 中活跃的 `Menu.*`/`menu.*` 资源读取检出 383 个菜单资源键和 473 处引用，现有矩阵关联 93 个键；12 个活跃硬编码菜单字面量和两个显式 `setAccelerator`（`Shift+O`、仅 Windows 的 `Alt+O`）均已全部反向关联。十四个文件的十五个已纳入键源合计 143/143 个活跃 `VK_*` case、365/365 条路径；二十六个文件的三十七个 pointer 源合计 111/111 个事件、443/443 条可达路径；全部反向关联 50 行矩阵并保留来源、修饰键、条件方法副作用、早退、空监听/空动作、执行语句、switch fall-through、数据依赖循环和原子 try/catch。代码库另有 6 个含键事件方法的文件和 4 个含鼠标事件方法的文件未纳入；其余 290 个菜单资源键也仍待盘点。
...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、二十六个文件的三十七个 pointer 来源共 111 个事件/443 条可达路径，并建立 50 个 `已盘点` 用例；当前关联 146 配置键、93 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`BOARD-MAIN-INPUT-001` 现覆盖 IndependentMainBoard 三个窗口按钮/窗口与主 `Input` 的五个来源共 22 个事件/273 条路径，贯通 mainPanel/toolbar/input2 路由、数子/选择/插入/标记/拖子/评论焦点/引擎对局/右键/中键/胜率图/自动变化和菜单控制面，并登记跨组件坐标、四副棋盘漏 return、滚轮去重、非原子棋谱编辑、线程取消、DPI/触控/键盘等价/无障碍与生命周期边界；`ENGINE-LIFECYCLE-001` 已覆盖 MoreEngines 的管理入口、唯一单击路径、未保存导航、引擎目录刷新、凭据/进程/窗口恢复安全边界，`UI-LAYOUT-001` 已覆盖 IndependentSubBoard，`ONLINE-YIKE-001` 已覆盖 WebView/YikeLiveDialog，`TSUMEGO-001` 已覆盖屏幕取题，`UI-TEXT-EDIT-001` 已覆盖共享文本控件，`UI-BLUNDER-SCROLLBAR-001` 已覆盖问题手列表与侧栏，`UI-WINDOW-MENU-STRIP-001` 已覆盖菜单壳，`UI-DETAILED-TOOLBAR-001` 已覆盖详细工具栏，`SETTINGS-GENERAL-001`/`SETTINGS-THEME-001` 已覆盖 ConfigDialog2 已纳入的现代设置输入；下一步盘点 `LizzieFrame.java`，之后处理其余三个 pointer 文件和六个键事件文件，再继续 450 配置键、290 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `LizzieFrame.java` 的 pointer 路径与主窗口安全边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
