...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十五个文件的十六个键盘来源共 144 个 case/367 条路径、三十个文件的八十三个 pointer 来源共 180 个事件/599 条可达路径，并建立 54 个 `已盘点` 用例；当前关联 214 配置键、101 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径，旧源码中的 active public pointer handler 已全部纳入。`BottomToolbar.java` 的走子编号 Enter 已贯通 `checkMove`、分支/主线跳转和 Board 导航，登记非法/零/负数/溢出边界、全局 toolbar 耦合、与按钮的引擎对局/评论编辑/焦点差异、同步 EDT 穿行、DPI/IME/无障碍及零入口级自动化测试；空 `keyReleased`/`keyTyped` 不伪装成活跃 case。下一步唯一盘点剩余五个键事件文件中的 `GtpConsolePane.java`，之后依次处理 `LizzieFrame.java`、`Menu.java`、`MoveListFrame.java`、`TsumeGoFrame.java`，再继续 382 配置键、282 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
