...
- T-003 按“活跃字面量叶键 + 明确持久化 JSON 接收者”的当前静态口径检出 596 个配置键和 1,627 处引用，现有矩阵关联 83 个键；另有 37 个仅出现在 `Config.java` 注释中的历史候选键；其余 513 个字面量键、嵌套规范路径、计算键和逐用例语义归属仍未完成。
- T-003 按 `Menu.java` 中活跃的 `Menu.*`/`menu.*` 资源读取检出 383 个菜单资源键和 473 处引用，现有矩阵关联 58 个键；12 个活跃硬编码菜单字面量和两个显式 `setAccelerator`（`Shift+O`、仅 Windows 的 `Alt+O`）均已全部反向关联。十四个文件的十五个已纳入键源合计 143/143 个活跃 `VK_*` case、365/365 条路径；十八个文件的二十一个 pointer 源合计 67/67 个事件、124/124 条可达路径；全部反向关联 49 行矩阵并保留来源、修饰键、条件方法副作用、早退、空监听/空动作、执行语句、switch fall-through、数据依赖循环和原子 try/catch。代码库另有 6 个含键事件方法的文件和 12 个含鼠标事件方法的文件未纳入；其余 325 个菜单资源键也仍待盘点。
...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、十八个文件的二十一个 pointer 来源共 67 个事件/124 条路径，并建立 49 个 `已盘点` 用例；当前关联 83 配置键、58 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`ONLINE-YIKE-001` 已补入 WebView 与 YikeLiveDialog 列表请求/取消竞态、分页、输入、代理、同步改盘/分析、窗口/网络/资源上限和无障碍边界，`TSUMEGO-001` 已补入屏幕取题权限/进程/棋盘事务边界，`UI-TEXT-EDIT-001` 已覆盖共享文本控件及 JPaintTextPane 的纯文本/HTML/缓存边界，`UI-BLUNDER-SCROLLBAR-001` 已覆盖 DemoScrollBarUI2 上下箭头 hover 及下游副作用，`UI-WINDOW-MENU-STRIP-001` 已覆盖顶级菜单壳的打开/关闭/悬停切换、陈旧菜单、重复通知、输入抑制、焦点和 listener 生命周期边界；下一步盘点 `IndependentSubBoard.java`，之后处理其余输入文件，再继续 513 配置键、325 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `IndependentSubBoard.java` 的 pointer 与窗口拖放边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
