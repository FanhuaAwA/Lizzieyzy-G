...
- T-003 按 `Menu.java` 中活跃的 `Menu.*`/`menu.*` 资源读取检出 383 个菜单资源键和 473 处引用，现有矩阵关联 48 个键；12 个活跃硬编码菜单字面量和两个显式 `setAccelerator`（`Shift+O`、仅 Windows 的 `Alt+O`）均已全部反向关联。四个已纳入键源合计 117/117 个活跃 `VK_*` case、333/333 条互斥路径；`InputIndependentSubboard.java` 与 `InputSubboard.java` 另有 12/12 个 pointer 事件、19/19 条互斥路径；全部反向关联 42 行矩阵并保留来源、修饰键、条件方法副作用、早退、空动作、执行语句和 switch fall-through。代码库另有 16 个含键事件方法的文件和 28 个含鼠标事件方法的文件未纳入；其余 335 个菜单资源键也仍待盘点。
...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及四个键源的 117 个 case/333 条路径和两个子棋盘源的 12 个 pointer 事件/19 条路径，并建立 42 个 `已盘点` 用例；当前关联 36 配置键、48 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；提交 `cf5cf53c01290f48086767789d56657fad183c5f` 已推送到 draft PR `#2`；提交 `70f78ae2b6a1fe4916ae04c6f0a8a61abdd037d2` 已形成，补入子棋盘输入并新增引擎对局控制和副棋盘输入用例；下一步盘点 `FloatBoard` 的键盘、鼠标、移动和滚轮入口，再处理其余 15 个键事件文件和 27 个鼠标事件文件，之后继续 560 配置键、335 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `FloatBoard` 的键盘、鼠标按下/离开、移动和滚轮入口并反向关联矩阵；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
