...
- T-003 按 `Menu.java` 中活跃的 `Menu.*`/`menu.*` 资源读取检出 383 个菜单资源键和 473 处引用，现有矩阵关联 102 个键；12 个活跃硬编码菜单字面量和两个显式 `setAccelerator`（`Shift+O`、仅 Windows 的 `Alt+O`）均已全部反向关联。十七个文件的十九个已纳入键源合计 147/147 个活跃 case、370/370 条路径；三十个文件的八十三个 pointer 源合计 180/180 个事件、599/599 条可达路径；全部反向关联 54 行矩阵并保留来源、修饰键、条件方法副作用、早退、空监听/空动作、执行语句、switch fall-through、数据依赖循环和原子 try/catch。旧源码 active public pointer handler 已全部纳入，另有 3 个含键事件方法的文件尚未盘点；其余 281 个菜单资源键也仍待盘点。
...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十七个文件的十九个键盘来源共 147 个 case/370 条路径、三十个文件的八十三个 pointer 来源共 180 个事件/599 条可达路径，并建立 54 个 `已盘点` 用例；当前关联 219 配置键、102 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径，旧源码中的 active public pointer handler 已全部纳入。`LizzieFrame.java` 的引擎对局专用 `gtpShortKey` 已贯通 `EngineManager` 的挂卸、常态/引擎对局 E 修饰键差异、Manual 人工落子双处理和主面板焦点边界；登记未 consume modifier、重复或缺失 listener、普通 `boolean noInput` 竞态和 Manual 连续双 toggle 等不得迁移的缺陷，KeyAdapter 空回调不伪装成活跃 case。下一步唯一盘点剩余三个键事件文件中的 `Menu.java`，之后依次处理 `MoveListFrame.java`、`TsumeGoFrame.java`，再继续 377 配置键、281 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，唯一盘点 `Menu.java` 的键事件路径与菜单动作安全边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
