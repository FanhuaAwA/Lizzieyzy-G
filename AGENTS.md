...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃字面量叶键/1,627 引用、37 注释候选键、373 菜单资源键/463 活跃引用，并建立 17 个 `已盘点` 高风险用例，当前仅 28 键关联用例；下一步完成 568 键反向映射、嵌套/计算键归一、完整菜单/快捷键/工作流拆分、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
