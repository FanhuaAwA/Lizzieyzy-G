...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十四个文件的十五个键盘来源共 143 个 case/365 条路径、二十八个文件的七十五个 pointer 来源共 168 个事件/557 条可达路径，并建立 53 个 `已盘点` 用例；当前关联 181 配置键、99 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`Menu.java` 的 5 个贴目索引来源完整覆盖 9 个 public pointer handler/9 条路径，贯通文本移出提交、按钮单击/长按、单双引擎命令、计分差异、changedKomi、条件 hover 与顶部工具栏可见性，并登记裸线程/丢失 release/重复 press、任意按钮、左键额外步进、非有限/无界输入、双引擎部分提交、默认焦点与动态无障碍应用时序等边界；下一步盘点 `MoveListFrame.java`，之后处理 `RightClickMenu.java` 和六个键事件文件，再继续 415 配置键、284 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `MoveListFrame.java` 的 pointer 路径与窗口安全边界；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
