...
- T-003 按 `Menu.java` 中活跃的 `Menu.*`/`menu.*` 资源读取检出 383 个菜单资源键和 473 处引用，现有矩阵关联 47 个键；12 个活跃硬编码菜单字面量中 6 个语言项已反向关联；两个显式 `setAccelerator`（`Shift+O`、仅 Windows 的 `Alt+O`）均已反向关联。`Input.java` 的 `keyPressed`/`keyReleased` 现有 56 个活跃 `VK_*` case（54/2），其中 15 个 case 已关联现有矩阵行；其余资源键、字面量、case 的工作流归属，以及 modifier/fall-through/动作组合和鼠标绑定仍待盘点。
...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量和 56 个 `Input.java` 键 case，并建立 17 个 `已盘点` 高风险用例；当前关联 28 配置键、47 菜单资源键、6 个菜单字面量、全部 2 个显式 accelerator 和 15 个输入 case；首批基线提交 `5598da03fed06f437c68f8d3d1cef6f86c220a19`、菜单索引提交 `8736e2b0f28a54fae7bfd545565f514bc9377b7e` 与输入索引提交 `d976c852b94843258503ec4b3b0a6811af18dd37` 已形成，堆叠 draft PR `#2` 为 OPEN/CLEAN；下一步完成 568 配置键、336 菜单资源键、6 个菜单字面量和 41 个输入 case 的工作流归属，归一 modifier/fall-through/动作组合、嵌套/计算键，并补 SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，将 6 个未映射硬编码菜单动作（详细工具栏、检查更新、清除个人数据）拆成稳定矩阵行，并归一 `Input.java` modifier/fall-through/动作组合；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
