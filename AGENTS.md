...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用，并建立 17 个 `已盘点` 高风险用例；当前关联 28 配置键、47 菜单资源键和全部 2 个显式 accelerator；首批基线提交 `5598da03fed06f437c68f8d3d1cef6f86c220a19` 与菜单索引提交 `8736e2b0f28a54fae7bfd545565f514bc9377b7e` 已推送，堆叠 draft PR `#2` 为 OPEN/CLEAN；下一步完成 568 配置键和 336 菜单资源键反向映射、嵌套/计算键归一、硬编码菜单/其他输入绑定与完整工作流拆分、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
