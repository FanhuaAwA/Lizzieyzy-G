...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十三个文件的十四个键盘来源共 142 个 case/363 条路径、十一个文件的十三个 pointer 来源共 39 个事件/85 条路径，并建立 46 个 `已盘点` 用例；当前关联 51 配置键、53 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；`ONLINE-YIKE-001` 已补入 BrowserFrame 的地址 Enter、载入/停止及标签视觉 pointer、JCEF bundle/cache/log、任意导航、页内 hook/探针、cefQuery bridge、OnlineDialog 会话/几何/SGF 和完整安全/生命周期迁移边界；下一步盘点剩余输入文件中最小的 `CaptureTsumeGoFrame.java`，之后处理其余输入文件，再继续 545 配置键、330 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `CaptureTsumeGoFrame.java` 的键盘入口并反向关联矩阵；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
