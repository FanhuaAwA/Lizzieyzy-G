...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及十一个文件的十二个键盘来源共 140 个 case/361 条路径、九个 pointer 来源共 32 个事件/76 条路径，并建立 46 个 `已盘点` 用例；当前关联 50 配置键、52 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；提交 `2d3a4441e4c51beea15fa4b26b3872b124f49b2e` 扩充 `ONLINE-TENCENT-001`，补入 TencentKifuDownload 的 Enter、表格双击、工具栏入口、代理/自动快速分析配置、联网/SGF/生命周期与失败边界，位于 draft PR `#2`；下一步盘点剩余文件中最小且同时含键盘和鼠标监听器的 `FoxKifuDownload.java`，之后处理其余输入文件，再继续 546 配置键、331 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `FoxKifuDownload.java` 的键盘和 pointer 入口并反向关联矩阵；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
