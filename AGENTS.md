...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃字面量叶键/1,627 引用、37 注释候选键、373 菜单资源键/463 活跃引用，并建立 17 个 `已盘点` 高风险用例，当前仅 28 键关联用例；实现提交 `5598da03fed06f437c68f8d3d1cef6f86c220a19` 已推送，堆叠 draft PR `#2` 为 OPEN/CLEAN；下一步完成 568 键反向映射、嵌套/计算键归一、完整菜单/快捷键/工作流拆分、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003 的配置反向映射、完整行为拆分与动态证据；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
...
| 2026-07-13 | T-003 旧版可执行基线与首批矩阵 | Apache Maven 3.9.16 ZIP 的 SHA-512 与官方值一致；仓库外隔离副本以 Temurin 21.0.11 执行 `mvn -Dfmt.skip=true test`，1,161 tests、0 failure、0 error、1 个 Windows 条件 skip；原旧源码无 `target`，576 个源文件逐一比对 0 差异，聚合指纹 `4615770949982e1d0742b7e6009be25e3345d97946d98ac243af65e5d98f4f23`；`python scripts/generate_legacy_inventory.py --check` 通过，清单含 276 主 Java、145 测试 Java、596 活跃配置叶键/1,627 引用、37 注释候选键、373 菜单资源键/463 活跃引用；矩阵首批 17 个用例均为 `已盘点`，关联 28 配置键；新版 locked restore、format verify、Release build 均退出 0，build 为 0 warning/0 error，5 项测试全通过，Release 窗口再次实测打开 19×19 空棋盘并正常关闭；实现提交 `5598da03fed06f437c68f8d3d1cef6f86c220a19` 已推送，堆叠 draft PR `https://github.com/FanhuaAwA/Lizzieyzy-G/pull/2` 以 T-002 分支为 base 且为 OPEN/CLEAN | T-003 未完成：568 个活跃叶键尚未关联用例，嵌套/计算键尚未归一，完整功能/快捷键拆分、SGF/GIB 与 GTP 语料、旧版动态界面截图、性能和跨实现验证均待完成；任何新版功能均未因本轮盘点而标为已实现或已验证；当前 PR 无 checks，原因是 T-006 尚未建立工作流，无 Actions run URL |
