...
| T-003 | 进行中 | 建立旧版行为基线和功能等价矩阵 | 已校验便携 Maven 并在隔离副本跑通旧版 1,161 项测试；已生成 576 文件来源指纹、596 活跃配置叶键/1,627 引用、37 注释候选键、383 菜单资源键/473 活跃引用、12 个硬编码菜单字面量，以及六个文件的七个键盘来源共 138 个 case/359 条路径、四个 pointer 来源共 22 个事件/57 条路径，并建立 44 个 `已盘点` 用例；当前关联 38 配置键、48 菜单资源键、全部菜单字面量/accelerator 和已纳入输入源的全部 case/事件/路径；提交 `2b658119131641850f1697a64a2bba87bddabc99` 补入 FloatBoard 输入，提交 `402676ba91656c66d0821b3197c39b1465a55d8d` 补入 AnalysisFrame 表格/窗口键盘、鼠标移动/拖动/离开/点击、滚轮和表头释放入口及 `UI-ANALYSIS-FRAME-001`，均已推送到 draft PR `#2`；下一步盘点范围小且同时含有效键盘和鼠标入口的 `DrawPainting.java`，之后处理其余输入文件，再继续 558 配置键、335 菜单资源键、嵌套/计算键、SGF/GIB 与 GTP 语料、旧版动态截图及 GUI/引擎分离性能 |
...
当前可用范围仅为能启动、显示 19×19 空棋盘并正常关闭的原生新壳；不包含棋谱、设置迁移、引擎或其他旧版功能，不能替代旧版。下一步继续 T-003，盘点 `DrawPainting.java` 的 Escape、鼠标释放/拖动/移动入口并反向关联矩阵；后续任何部分功能必须新增/拆分 ID，不得藏在“进行中”描述里。
